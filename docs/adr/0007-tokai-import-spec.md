...
| 事業者 | Wikidata Q-ID | 略称 | 主な対象路線数 (推定) |
|---|---|---|---|
| 東海旅客鉄道 (JR東海) | Q177489 | JR東海 | 12 |
| 東日本旅客鉄道 (JR東日本) | Q103982 | JR東日本 | 1〜2 (静岡県東部のみ) |
| 西日本旅客鉄道 (JR西日本) | Q170448 | JR西日本 | 1 (関西本線亀山以西の三重県内) |
| 名古屋鉄道 | Q244105 | 名鉄 | 22 |
| 近畿日本鉄道 | Q243056 | 近鉄 | 7 |
| 名古屋市交通局 | Q11648001 | 名古屋市営地下鉄 | 6 |
| 名古屋臨海高速鉄道 | Q11346002 | あおなみ線 | 1 |
| 愛知高速交通 | Q11321002 | リニモ | 1 |

合計 **約 50 路線** (見込み)。バスは対象外 (本 ADR の scope 外, 将来別 ADR で検討)。
...
1. allowlist に含まれる事業者が運営する **鉄道路線** (`P31 wd:Q728937` など鉄道路線系クラス + `P127` ∈ allowlist) を SPARQL で取得
2. 各路線について, 4 県のいずれかに属する駅 (`P131*` ∈ {4県} を P131 階層で再帰的に解決) を取得
3. **路線が完全に対象県外** (= マッチする駅が 1 件もない場合) はその路線を除外
4. 駅の所属路線 (P81) のうち, allowlist 外の事業者が運営する路線への接続は `StationLine` テーブルに含めない (例: 名古屋駅から地下街経由の他社線は除外)
...
**含める**。allowlist に JR 東日本を入れたため, 熱海以東の静岡県内駅 (熱海駅, 来宮駅) も自動的に対象となる。「JR」と一括りに考える利用者の自然な期待に沿う。
...
- JR 西日本 (関西本線亀山以西) と JR 東日本 (静岡県東部) は数路線・数駅しか含まれないため, 「JR東海以外も入っているのか?」と利用者が混乱する可能性。スクリプトの取り込みサマリと, 必要であれば README で明示する
