...
### D. 駅名正規化と article overrides (fix-it-forward)

実取り込みで以下の 2 つの取りこぼしが発覚し追加修正:

1. **駅名正規化の不一致**: Wikipedia 由来の "近鉄名古屋駅" は `normalizeWikipediaStationName` が末尾「駅」のみ除去して "近鉄名古屋" になっていたが, DB 側 (`normalizeStationName`) はプレフィックス + 末尾「駅」両方を除去して "名古屋" に統一されている。同じ前処理パターン (`NAME_PREFIX_PATTERNS`) を Wikipedia 側にも適用するよう修正。これで「近鉄名古屋駅」「JR東海名古屋駅」「名鉄名古屋駅」全て "名古屋" に正規化されてマッチする。
2. **Umbrella article で駅番号 wikitable が無いケース**: 関西本線 (Q1199703) の Wikidata sitelink が「関西本線」全国版記事を指し, この記事は umbrella で駅番号一覧表が無かった (sub-article 「関西線 (名古屋地区)」に分かれている)。`LINE_ARTICLE_OVERRIDES` マップを `wikipedia-line-pages.ts` に追加し, `Q1199703 → "関西線 (名古屋地区)"` のようなオーバーライドを許容。
