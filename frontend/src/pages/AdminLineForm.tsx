...
export function AdminLineForm({ mode }: AdminLineFormProps) {
  ...
  // US-049: operator は運営会社マスタの dropdown で選択させる。
  ...
  /// 互換用フリーテキスト (operatorId 未設定の旧レコード表示用)。
  const [formOperator, setFormOperator] = useState('')
  /// US-049: Operator マスタへの FK (推奨)。
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="group">
          ...
          <select
            ...
            onChange={(e) => {
              setFormOperatorId(e.target.value)
              // dropdown 選択時に互換用 operator 文字列も同期 (UI 表示用)。
              const op = operatorsState.operators?.find((o) => o.id === e.target.value)
              setFormOperator(op?.name ?? '')
            }}
